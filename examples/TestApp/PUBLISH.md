...
This directory contains configurable scripts to publish TestApp as an optimized single-file executable with optional features.
...
publish/
├── linux-x64/
│   ├── TestApp          # Single executable (~69MB, includes SDL/GLFW)
...
publish/
├── linux-x64/
│   ├── TestApp          # Single executable (~71MB, includes everything)
│   └── *.pdb                    # Debug symbols (optional)
├── osx-x64/
│   └── TestApp
└── win-x64/
...
3. **Verify the configuration**: Ensure `IncludeNativeLibrariesForSelfExtract=true` in TestApp.csproj (this is now set to true with PathResolver support)
...
```yaml
- name: Publish TestApp
  run: dotnet publish examples/TestApp/TestApp.csproj -c Release -r linux-x64 -o dist/ --self-contained false /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true
```
