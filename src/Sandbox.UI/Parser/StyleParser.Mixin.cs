...
internal static partial class StyleParser
{
	...
	private static List<MixinParameter> ParseMixinParameters( string content, string fileAndLine )
	{
		...
		foreach ( var part in parts )
		{
			...
			if ( colonIndex >= 0 )
			{
				var paramName = trimmed.Substring( 1, colonIndex - 1 ).Trim();
				var defaultValue = trimmed.Substring( colonIndex + 1 ).Trim();
				parameters.Add( new MixinParameter( paramName, defaultValue, isVariadic ) );
			}
			else
			{
				var paramName = trimmed.Substring( 1 ).Trim();
				parameters.Add( new MixinParameter( paramName, isVariadic ? "" : null, isVariadic ) );
			}
		}
	...
	private static Dictionary<string, string> ParseIncludeArguments( string content, MixinDefinition mixin, string fileAndLine )
	{
		...
		foreach ( var part in parts )
		{
			...
			if ( trimmed.StartsWith( '$' ) && trimmed.Contains( ':' ) )
			{
				namedStarted = true;
				var colonIndex = trimmed.IndexOf( ':' );
				var argName = trimmed.Substring( 1, colonIndex - 1 ).Trim();
